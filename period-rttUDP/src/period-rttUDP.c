...
static void usage(void){
	printf("\nrttUDP v1.3 round-trip time UDP tester.\n"
		...
		"Options:\n"
		"  SO_TS - request kernel space software\n"
		"  time stamps to calculate latency (SO_TIMESTAMPING)\n"
		"  CLK_GT - request user space\n"
		"  time stamps using clock_gettime() to calculate latency \n"
		"  ALL - request both time stamp reporting methods and calculate latency\n"
		"  BIN - (preferred) requests kernel space timestamping\n"
		"  to be written into a binary file (default output.bin)\n");
	exit(EXIT_FAILURE);
}
...
void* cyclicTask(void* arg) {
	...
	else if(strcmp(args->option,"BIN")==0){
		opt_flag=4;
	}
	else
		opt_flag=0;
    ...
    while ((args->num_pkg==-1 || sent_pkgs<args->num_pkg)&& !exit_flag) {
        ...
        //Tx
    	if(opt_flag==4||opt_flag==1||opt_flag==3){
        ...
        uint16_t lat_us = lat_ts.tv_nsec/1000;
        if(opt_flag==4){
        	fwrite(&lat_us, sizeof(lat_us), 1, file);
        }
        else{
